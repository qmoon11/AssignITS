# References

If you use ClassifyITS in your research or publications, **please cite this package** as:

Moon, Q. (2026). ClassifyITS: Quality control, taxonomy assignment, and visualization of fungal OTU representative sequences based on BLAST results. R package, version 0.1.0. [https://github.com/qmoon/ClassifyITS](https://github.com/yourusername/ClassifyITS)
*this will be updates as the package is published on CRAN*


Below are key citations and resources relevant to the ClassifyITS pipeline and fungal ITS taxonomy assignment.

---

## Why use ITS for Fungal Identification?

- Schoch, C.L., et al. (2012). Nuclear ribosomal internal transcribed spacer (ITS) region as a universal DNA barcode marker for Fungi. *Proceedings of the National Academy of Sciences*, 109(16): 6241-6246.  
  https://doi.org/10.1073/pnas.1117018109

---

## UNITE Reference Database

- Nilsson, R.H., et al. (2019). The UNITE database for molecular identification of fungi: handling dark taxa and parallel taxonomic classifications. *Nucleic Acids Research*, 47(D1): D259-D264.  
  https://doi.org/10.1093/nar/gky1022

- UNITE Community: UNITE Database website  
  https://unite.ut.ee/

---

## BLAST and Sequence Alignment

- Camacho, C., et al. (2009). BLAST+: architecture and applications. *BMC Bioinformatics*, 10:421.  
  https://doi.org/10.1186/1471-2105-10-421

---

## Best Practices in Fungal ITS Analysis

- Tedersoo, L., et al. (2021). Best practices in DNA metabarcoding of fungi: an update. *Molecular Ecology Resources*, 21(3): 599-620.  
  https://doi.org/10.1111/1755-0998.13293

---

## Amplicon Processing Pipelines

- Callahan, B.J., et al. (2016). DADA2: High-resolution sample inference from Illumina amplicon data. *Nature Methods*, 13:581–583.  
  https://doi.org/10.1038/nmeth.3869

- Frøslev, T.G., et al. (2017). Algorithm for post-clustering curation of DNA amplicon data yields reliable biodiversity estimates. *Nature Communications*, 8:1188.  
  https://doi.org/10.1038/s41467-017-01312-x (LULU algorithm)

- QIIME2: Bolyen, E., et al. (2019). Reproducible, interactive, scalable and extensible microbiome data science using QIIME 2. *Nature Biotechnology*, 37:852–857.  
  https://doi.org/10.1038/s41587-019-0209-9

---

## Importantly: Original Sources for Unique Cutoffs per Taxonomic Level and example application of the cutoffs

- Tedersoo, L., et al. (2014). Global diversity and geography of soil fungi. *Science*, 346(6213): 1256688.  
  https://doi.org/10.1126/science.1256688

- Nilsson, R.H., et al. (2019). Best practices in metabarcoding of fungi: From experimental design to results interpretation. *Molecular Ecology*, 28(3): 185-206.  
  https://doi.org/10.1111/mec.14341

- Tedersoo, L., et al. (2021). The Global Soil Mycobiome consortium dataset for boosting fungal diversity research. *Fungal Diversity*, 109: 1-9.  
  https://doi.org/10.1007/s13225-021-00480-4

---

## Additional Community Resources

- NCBI BLAST+ Documentation  
  https://blast.ncbi.nlm.nih.gov/Blast.cgi?CMD=Web&PAGE_TYPE=BlastDocs

- ITSx: Extraction of ITS regions from fungal sequences  
  https://github.com/ITSx/ITSx